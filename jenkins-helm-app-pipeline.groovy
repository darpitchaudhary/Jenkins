pipeline {
  ...
  stages {
    ...
    stage('Helm install') {
                steps {
                    withCredentials([file(credentialsId: 'kube2', variable: 'config')]) {
                      sh '''
                        imagename="${registry}:$(git rev-parse HEAD)"
                        export KUBECONFIG=\${config}
                        helm upgrade release k8s_chart --install --set app.container.image=$imagename --set imageCredentials.username="darpitchaudhary" --set imageCredentials.password="docker pwd" --set postgres.config.data[0].key="host" --set postgres.config.data[0].value="rds-webapp.c8xrccb2wii3.us-east-1.rds.amazonaws.com" --set postgres.config.data[1].key="name" --set postgres.config.data[1].value="postgres"  
                      '''
